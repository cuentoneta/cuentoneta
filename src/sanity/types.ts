...
// Source: ../src/api/_queries/collection.query.ts
// Variable: collectionTeasersQuery
// Query: *[_type == 'collection' && !(_id in path('drafts.**'))]| order(title asc){    _id,    'slug': slug.current,    title,    description,    featuredImage,    'config': { 'showAuthors': coalesce(config.showAuthors, false) },    'tags': coalesce(tags[] -> {        title,        'slug': slug.current,        description    }, []),    'mediaSources': coalesce(mediaSources[]{        ...,        _type == 'spaceRecording' => {            'audioUrl': audioFile.asset->url        }    }, []),    'count': count(literaryWorks),    'literaryWorkCoverImages': coalesce(literaryWorks[0...3]->coverImage, [])}
export type CollectionTeasersQueryResult = Array<{
	_id: string;
	slug: string;
	title: string;
	description: Markdown;
