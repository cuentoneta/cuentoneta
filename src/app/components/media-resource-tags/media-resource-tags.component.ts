...
@Component({
	...
	template: ` @for (mediaResource of resources(); track $index) {
		@switch (mediaResource.type) {
			@case ('audioRecording') {
				<div [title]="'Contiene narraciones en audio'" [class]="size()">
					<img [alt]="'Contiene narraciones en audio'" src="assets/svg/waveform.svg" />
				</div>
			}
			@case ('spaceRecording') {
				<div [title]="'Contiene grabaciones de Spaces de X'" [class]="size()">
					<img [alt]="'Contiene grabaciones de Spaces de X'" src="assets/svg/twitter.svg" />
				</div>
			}
			@case ('youTubeVideo') {
				<div [title]="'Contiene videos de YouTube'" [class]="size()">
