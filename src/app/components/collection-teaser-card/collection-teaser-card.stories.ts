...
const meta: Meta<CollectionTeaserCard> = {
	...
	parameters: {
		docs: {
			...
			description: {
				component: `<div><p>El <strong>CollectionTeaserCard</strong> es la tarjeta de una colección para el Design System v3, tipada contra el modelo de dominio <strong>Collection</strong>: portada, título, descripción y footer con tag y contador de obras. La portada se resuelve con el objeto de valor <strong>imagery</strong>: <strong>representative</strong> (una portada editorial propia de la colección) o <strong>sample</strong> (composición de 3 portadas de sus obras, con placeholder en los slots vacíos). Usa <a href="./?path=/docs/componentes-v3-coverimage--docs" target="_top"><strong>CoverImage</strong></a> para cada portada.</p><p>La descripción llega del backend como HTML ya saneado y se pinta con <code>[innerHTML]</code>, porque el pipeline emite su propio <code>&lt;p&gt;</code>. Sustituye a <a href="./?path=/docs/componentes-v3-storylistteasercard--docs" target="_top"><strong>StorylistTeaserCard</strong></a>, que muestra lo mismo desde el agregado en baja y sigue sirviendo la home; las dos comparten el skeleton, que no depende del modelo.</p></div>`,
			},
		},
	},
	argTypes: {
