...
/**
 * Barra de carga (skeleton) in-house que reemplaza a `ngx-skeleton-loader`. El host ES la barra: el
 * consumidor controla alto, ancho y color con clases utilitarias de Tailwind directamente sobre el
 * elemento (`<cuentoneta-skeleton class="h-[36px] w-[40px] bg-brand-300" />`), sin `::ng-deep` ni `@apply`.
...
@Component({
	...
	host: {
		...
		'[class.rounded-sm]': "appearance() === 'square'",
