...
describe('StoryEditionDateLabelComponent', () => {
	const renderComponent = async (inputs: {
		label?: string;
		markAsNew?: boolean;
	}): Promise<RenderResult<StoryEditionDateLabelComponent>> => {
		return render(StoryEditionDateLabelComponent, { inputs });
	};

	it('should create', async () => {
		const { container } = await renderComponent({ label: 'Día 1' });
		expect(container).toBeTruthy();
	});

	it('should display label when provided', async () => {
		await renderComponent({ label: 'Día 1' });
	...
	it('should display "NUEVO" badge when markAsNew is true', async () => {
		await renderComponent({ label: 'Día 1', markAsNew: true });
	...
	it('should not display "NUEVO" badge when markAsNew is false', async () => {
		await renderComponent({ label: 'Día 1', markAsNew: false });
	...
	it('should display both label and badge together', async () => {
		await renderComponent({ label: 'Día 5', markAsNew: true });
	...
	it('should render visual indicator (colored bar) with label', async () => {
		await renderComponent({ label: 'Día 10' });

		const visualIndicator = screen.getByTestId('visual-indicator');
