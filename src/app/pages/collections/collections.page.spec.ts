...
describe('CollectionsPage', () => {
	...
	it('should order titles with accent folding, not by code point', async () => {
		const porPuntoDeCodigo = [...onoffCollectionTeasersMock].sort((one, other) => (one.title < other.title ? -1 : 1));
		const [conAcentoInicial] = onoffCollectionTeasersWithNonAsciiInitialMock;
	...
	describe('filtros', () => {
		const catalogo = onoffCollectionTeasersMock;
		...
		const carrying = (...tags: readonly Tag[]) =>
			catalogo.filter((collection) =>
				tags.every((tag) => collection.tags.some((candidate) => candidate.slug === tag.slug)),
			);

		const facetFor = (tag: Tag, over: readonly CollectionTeaser[] = catalogo) => {
		...
		it('should narrow the listing to the collections carrying the chosen tag', async () => {
			...
			expect(hrefsOf(screen.getByTestId('collections'))).toHaveLength(conLaEtiqueta.length);
			expect(screen.getByRole('heading', { level: 1, name: headingFor(conLaEtiqueta) })).toBeInTheDocument();
		});
		...
		it('should offer a chip that removes the filter it names', async () => {
			...
			expect(screen.getByRole('heading', { level: 1, name: headingFor(catalogo) })).toBeInTheDocument();
		});
		...
		it('should collapse the category group without dropping the filters in effect', async () => {
			...
			expect(screen.queryByLabelText(facetFor(colaborativaTagMock, conLaEtiqueta))).not.toBeInTheDocument();
			expect(screen.getByRole('heading', { level: 1, name: headingFor(conLaEtiqueta) })).toBeInTheDocument();
		...
		it('should never let a combination of offered facets empty the listing', async () => {
			await renderCatalogo();
			const conLaPrimera = carrying(colaborativaTagMock);
			await userEvent.click(screen.getByLabelText(facetFor(colaborativaTagMock)));

			await userEvent.click(screen.getByLabelText(facetFor(ensayoTagMock, conLaPrimera)));

			const conLasDos = carrying(colaborativaTagMock, ensayoTagMock);
			expect(conLasDos).not.toHaveLength(0);
			expect(hrefsOf(screen.getByTestId('collections'))).toHaveLength(conLasDos.length);
			expect(screen.getByRole('heading', { level: 1, name: headingFor(conLasDos) })).toBeInTheDocument();
		});
	});
