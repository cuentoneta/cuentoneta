...
export const CatalogoDelCorpus: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: `<p>El catálogo del corpus, tal como lo sirve el backend. Es la entrada donde se ejercita el panel entero.</p><p>Qué mirar acá:</p><ul><li>Facetas con <strong>conteos distintos</strong>, que es lo que revela cuántas colecciones lleva cada etiqueta.</li><li>Al elegir una, las que <strong>no conviven</strong> con ella desaparecen y las que sí ajustan su número a lo que queda.</li><li>Elegir dos que conviven deja el cruce; como toda faceta ofrecida tiene al menos una colección detrás, <strong>no hay forma de vaciar el listado</strong>.</li><li>El encabezado sigue al resultado, porque es el conteo de lo que se está mostrando.</li><li>Una colección sin etiquetar no aparece bajo ningún filtro, que es lo que hace en producción.</li></ul><p>También es donde se evalúa el orden: <strong>Ámbar y ceniza</strong> aparece donde corresponde alfabéticamente y no al final, que es donde la pondría una comparación por punto de código.</p><p><strong>Usos:</strong> mirar la página con datos reales, sin entradas derivadas.</p>`,
			},
		},
	},
};
