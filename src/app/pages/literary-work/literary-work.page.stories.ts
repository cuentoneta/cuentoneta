...
const meta: Meta<LiteraryWorkPageArgs> = {
	...
	parameters: {
		...
		docs: {
			...
			description: {
				component: `<div><p>La página de lectura, ${docsMention(literaryWorkPageDocs)}, montada sobre el corpus de Onoff. Como el resto de las entradas bajo <strong>Páginas</strong>, no cataloga un componente sino el ensamblado completo, que es lo que permite mirar cómo conviven el encabezado, la barra de lectura, el bloque de formatos, el cuerpo y las sugerencias.</p><p>Los tres controles reproducen lo que la ruta le entrega a la página:</p><ul><li><code>slug</code> — qué obra del corpus se lee. Un slug que no existe cae en el estado de obra inexistente, y la entrada <code>— Cargando —</code> deja la página en su esqueleto.</li><li><code>navigation</code> y <code>navigationSlug</code> — el contexto con el que se llegó, que decide qué ${docsLink(readingSuggestionsListDocs, 'sugerencias de lectura')} se ofrecen al pie: las del autor o las de una colección.</li></ul><p>Los dos ejes que el diseño define se recorren desde acá: el <strong>tipo de multimedia</strong>, según cuántos recursos declare la obra —lo resuelve ${docsRef(mediaWidgetSelectorDocs)}—, y la <strong>forma de las sugerencias</strong>, según el contexto de navegación.</p><p>Las sugerencias viven en un bloque diferido por viewport, así que aparecen al bajar hasta el pie. El bloque de formatos también es diferido, pero dispara al quedar el hilo libre, así que en la práctica su esqueleto no llega a verse acá; está catalogado en las entradas de ${docsMention(mediaWidgetSelectorDocs)}.</p><p>Los destinos de los medios del corpus son ficticios, así que los reproductores no cargan contenido real: lo que se evalúa en estas entradas es la disposición.</p></div>`,
			},
		},
	},
	argTypes: {
