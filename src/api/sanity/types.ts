...
// Variable: landingPageContentReferencesQuery
// Query: *[_type == 'landingPage' && !(_id in path('drafts.**')) && slug.current == $slug][0]{    _id,    _type,    'slug': slug.current,    config,    'cards': coalesce(cards[],[]),    'campaigns': coalesce(campaigns[],[]),    'latestReads': coalesce(latestReads,[]),}
export type LandingPageContentReferencesQueryResult = {
	_id: string;
	_type: 'landingPage';
	slug: string;
	config: string;
