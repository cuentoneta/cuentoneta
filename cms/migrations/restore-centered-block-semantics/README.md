...
```bash
pnpm exec sanity documents query --api-version v2021-06-07 --project-id "$(node --env-file=.env -p 'process.env.SANITY_STUDIO_PROJECT_ID')" --dataset production \
...
```bash
pnpm exec sanity migration run restore-centered-block-semantics --project "$(node --env-file=.env -p 'process.env.SANITY_STUDIO_PROJECT_ID')" --dataset production > dry-run.txt
```
...
```bash
pnpm exec sanity migration run restore-centered-block-semantics --project "$(node --env-file=.env -p 'process.env.SANITY_STUDIO_PROJECT_ID')" --dataset production --no-dry-run --no-confirm
```
