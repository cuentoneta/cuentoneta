...
**Embeds de terceros: el catálogo monta una ilustración local.** `src/testing/storybook-embed-placeholders.ts` aporta `embedPlaceholdersDecorator` —que apaga la carga de la IFrame API de YouTube y repinta el placeholder que el player dibuja— y `withEmbedPlaceholder(s)`, que reapunta la URL del episodio de Spotify a un SVG servido desde `assets/`. Son dos mecanismos porque hay dos seams distintos: el episodio expone su URL como dato, el video no (su `videoId` no es una URL).
