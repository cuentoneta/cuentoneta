...
>
> \* Contrato en `docs/LITERARY_WORK_DESIGN.md` §7: el transporte es `LiteraryWorkDto` y `HttpLiteraryWorkApi` lo **rehidrata** al dominio con las mismas factories que el backend (`createLiteraryWork`, `createSanitizedHtml`, `createReadingTime`, …). Es un ACL de frontend simétrico al del repository: un DTO inválido lanza en el provider, no en un template.
