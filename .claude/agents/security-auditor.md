...
La columna **S#** numera de forma continua a través de las tres tablas (misma mecánica que la "Numeración de problemas" de [`code-reviewer.md`](code-reviewer.md)), con el prefijo `S` (S1, S2, …) para que un hallazgo de seguridad pueda citarse sin ambigüedad junto a los del `code-reviewer` (que llevan prefijo `R`). Ninguno de los dos usa `#`, que queda reservado para los issues de GitHub. El prefijo es **obligatorio en todo hallazgo** y el conjunto es **cerrado** (`R` y `S`, ningún otro): es lo que permite que un check los enumere.

La severidad **no** entra en el identificador: un hallazgo puede promoverse o descartarse durante la review, y si la severidad viviera en su ID, cambiarla renumeraría el hallazgo y rompería toda referencia previa. Por eso la numeración es continua a través de las tres tablas y la severidad vive en su propia columna. La numeración **no** continúa la de `CODE_REVIEW.md`: ambos agentes corren en paralelo en la Fase 4 del skill `issue-workflow`, así que este agente no puede conocer en qué número terminó el reviewer.
