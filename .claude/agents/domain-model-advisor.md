...
description: Revisa modelos de dominio de cuentoneta (LiteraryWork, Author, Collection, Resource) buscando patrones DDD — agregados e invariantes, value objects (Slug/ReadingTime/DateString), inmutabilidad, diseño interface-first y validación en frontera. Lo invoca la Fase 2 del skill issue-workflow —antes del plan-writer— cuando el issue crea o modifica entidades de dominio, value objects, mappers del ACL, queries GROQ o tipos de dominio compartidos; su evaluación alimenta el plan. También se puede invocar a demanda.
...
>
> **Contexto de madurez:** cuentoneta es hoy **"DDD-lite"** — capas `controller → service → repository` con un ACL de mappers sólido, y con `LiteraryWork`/`Collection` como raíces de agregado con invariantes hechas cumplir en código; specification y domain events como código siguen siendo **roadmap** (`docs/DDD_IMPROVEMENTS.md`, issue **#1503**). Distinguí siempre entre lo **vigente** (qué exigir hoy) y lo **objetivo** (qué recomendar como dirección).
...
- Se diseñan nuevas entidades de dominio o value objects (`LiteraryWork`, `Author`, `Collection`, `Resource`, `Tag`, `Media`, `Epigraph`, …).
...
1. `.claude/references/domain-model.md` — DDD estratégico y táctico re-ejemplificado con LiteraryWork/Author/Collection/Resource (es la guía principal).
...
1. **Identificar entidades de dominio** — qué entidades, value objects, vistas polimórficas o agregados (raíz `LiteraryWork` / `Author` / `Collection`) están en juego, y a qué **bounded context** pertenecen.
...
- [ ] Los componentes y services dependen de **interfaces de dominio** (`LiteraryWork`, `Author`, `Collection`, `Resource`), no del shape crudo de Sanity ni de DTOs de contrato.
- [ ] **Sin prefijo `I`:** la interfaz de dominio lleva el nombre limpio (`LiteraryWork`, `Author`) y **nunca** el prefijo `I`, sin excepciones. No se declara una clase homónima que duplique el contrato: las invariantes las hace cumplir la factory del mapper.
- [ ] La interfaz contiene todas las propiedades y métodos públicos del contrato.

### Vistas polimórficas (projection pattern) — vigente

- [ ] Un agregado expone **múltiples vistas** según el caso de uso (`LiteraryWork` / `LiteraryWorkTeaser` / `LiteraryWorkNavigationTeaser` / `LiteraryWorkNavigationTeaserWithAuthors`; `Author` / `AuthorTeaser`; `Collection` / `CollectionTeaser`).
...
- [ ] Mappers como **funciones puras** (`mapAuthor`, `mapResources`, `mapTags`, …): misma entrada → misma salida, sin I/O.
...
- [ ] **Agregados e invariantes:** `LiteraryWork` es la raíz del agregado de contenido, con sus invariantes hechas cumplir por `createLiteraryWork` (título no vacío, `>= 1` sección, `>= 1` autor, posiciones de sección contiguas); `Collection` hace lo propio con `createCollection` (título no vacío, `>= 1` obra, `count` derivado). `Author` es raíz de su propio agregado, con invariantes solo descritas — su enforcement en código sigue siendo roadmap.
...
- [ ] **Type-state / domain events:** recomendar solo cuando la divergencia entre estados o el acoplamiento entre contextos lo justifique (roadmap #1503). No inventar `DraftLiteraryWork`/`PublishedLiteraryWork` ni eventos como código sin que el issue lo pida.

### Convenciones de cuentoneta (de CLAUDE.md)

- [ ] **Sin `enum` de TS** — usar `Object.freeze({...} as const)` para conjuntos cerrados de literales (`ResourceType.slug`, `MediaTypeKey`, …).
...
├── <entidad>.interface.ts    # contrato de dominio (LiteraryWork, Author, …)
