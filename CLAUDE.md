...
| `pnpm issue-refs:sweep`                   | Verifica que los issues citados en los `.md` de `.claude/` y en los comentarios de `src/`/`cms/` sigan abiertos y reporta los que no; con `--apply` mantiene el issue de seguimiento. Lo corre un job programado, **no** un gate                |
...
- **Hook `PreToolUse` de `Edit`/`Write`** (versionado en `.claude/settings.json`): [`scripts/block-issue-refs-in-comments.ts`](scripts/block-issue-refs-in-comments.ts) rechaza escribir un comentario de código bajo `src/` o `cms/` que cite un issue, salvo las dos excepciones de [Comentarios](#comentarios), o que cite un identificador de hallazgo de review (`R1`, `S3`), que no tiene excepción. Es una **ayuda local, no un gate**: solo ve el texto que la herramienta agrega, no lo ya commiteado ni lo escrito por fuera.
