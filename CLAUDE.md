...
| `pnpm lint`                               | ESLint sobre `src`, `e2e`, `resources`, `cms` y `scripts`                                                                                                                                                                                                                                                                      |
| `pnpm check:agents`                       | Valida la integridad de `.claude/` (frontmatter de agentes y skills + anclas, rutas y menciones a issues de las referencias) — reproduce el gate `check-agents`                                                                                                                                                                |
| `pnpm issue-refs:sweep`                   | Verifica que los issues citados en los `.md` de `.claude/` y en los comentarios de `src/`/`cms/`/`scripts/` sigan abiertos y reporta los que no; con `--apply` mantiene el issue de seguimiento. Lo corre un job programado, **no** un gate                                                                                    |
| `pnpm required-fields:sweep`              | Cuenta los documentos que incumplen un campo que el schema declara requerido, derivándolos de `cms/schema.json`; con `--apply` mantiene el issue de seguimiento. Read-only sobre el contenido. Lo corre un job programado, **no** un gate                                                                                      |
| `pnpm worktrees:sweep`                    | Reporta los worktrees ya mergeados y los directorios huérfanos de `.claude/worktrees/`; con `--apply` remueve los registrados, nunca borra un huérfano                                                                                                                                                                         |
| `pnpm check:findings`                     | Verifica que un texto (mensaje de commit o cuerpo de PR, `--surface=commit\|pr`) no cite un identificador de hallazgo de review — lo usan el hook `commit-msg` y el gate `check-findings`                                                                                                                                      |
| `pnpm comments:inventory <ruta>…`         | Vuelca a JSON los comentarios de las rutas dadas (archivo, línea, tipo, texto) bajo `workspace/`. Lo consume el paso de inventario de la skill `aposd-comment-audit`. Reporta, **no** es un gate                                                                                                                               |
| `pnpm stylelint`                          | Stylelint sobre CSS                                                                                                                                                                                                                                                                                                            |
| `pnpm typecheck`                          | Type-check estricto (`tsc --noEmit`)                                                                                                                                                                                                                                                                                           |
| `pnpm test`                               | Tests unitarios (Vitest). Envuelve el target Nx **`vitest:test`**, inferido por el plugin `@nx/vitest` — no `test`, que es el nombre que Nx ya le da a este mismo script                                                                                                                                                       |
| `pnpm test:watch`                         | Tests en watch (invoca Vitest directo, sin Nx)                                                                                                                                                                                                                                                                                 |
| `pnpm test:ui`                            | Interfaz web de Vitest en watch (`@vitest/ui`), sin Nx                                                                                                                                                                                                                                                                         |
| `pnpm test:e2e`                           | E2E (Playwright)                                                                                                                                                                                                                                                                                                               |
| `pnpm corpus:generate`                    | Regenera las fixtures raw del corpus de mocks de Onoff (`literary-work`/`collection`/`landing-page`) evaluando la query GROQ real con `groq-js` sobre los documentos escritos a mano → [`src/mocks/onoff/README.md`](src/mocks/onoff/README.md)                                                                                |
...
| `pnpm seo:body-sweep`                     | Barre **todas** las URLs del sitemap afirmando que la página sirve un cuerpo con contenido, sin depender del muestreo del smoke. Lo corre un paso del job semanal de indexado, que mantiene su issue de seguimiento; **no** es un gate                                                                                         |
| `pnpm seo:index-status`                   | Mide el estado de indexado real contra la URL Inspection API de Search Console (requiere service account: `GSC_SERVICE_ACCOUNT_KEY_PATH` o `GSC_SERVICE_ACCOUNT_KEY`, más `GSC_SITE_URL`). Lo corre además un job programado semanal que versiona la serie en una rama de datos; como el sweep de menciones, **no** es un gate |
...
| `pnpm exec sanity migration run <slug>`   | Corre una migración de datos (desde `cms/`; dry-run por defecto). El destino va explícito con `--project` + `--dataset`, que son inseparables → [`sanity-migrations.md`](.claude/references/sanity-migrations.md)                                                                                                              |

**Gates de CI** (deben quedar verdes en cada PR): `test`, `lint`, `stylelint`, `typecheck`, `e2e`, `build`, `storybook`, `studio-build`, `guard-config`, `check-agents`, `check-findings`.

> Los once son **required status checks** de `develop`: GitHub bloquea el merge si alguno queda en rojo. El nombre del gate es el **id del job** en `.github/workflows/` — los jobs no declaran `name:` justamente para que id y context no puedan desincronizarse. `guard-config` solo tiene efecto en PRs desde forks (en PRs del mismo repo pasa en verde sin verificar nada).
...
>
> El gate **`check-findings`** (`.github/workflows/check-findings.yml`, workflow aparte de `ci.yml`) valida que los mensajes de commit del PR (vía la API de GitHub) y el cuerpo del PR no citen un identificador de hallazgo de review. Corre además en el evento `pull_request.edited` —el único de los gates que lo hace—, porque es la única forma de que corregir el cuerpo del PR vuelva a evaluar el check.
...
| --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Largo de función                                          | ≤ 50 líneas (ESLint `max-lines-per-function`; exentos `*.spec.ts` y `*.stories.ts` — ver abajo)                                                                                                                                                                                                                                                                |
| Largo de archivo                                          | ≤ 500 líneas (ESLint `max-lines`; exentos `*.spec.ts`, `*.stories.ts` y `src/sanity/types.ts`, que lo emite el typegen)                                                                                                                                                                                                                                        |
| Complejidad ciclomática                                   | ≤ 10 (ESLint `complexity`, **sin** exención: rige también en specs y stories)                                                                                                                                                                                                                                                                                  |
| Profundidad de anidamiento                                | ≤ 3 niveles (ESLint `max-depth`, **sin** exención)                                                                                                                                                                                                                                                                                                             |
...
| `z-index` crudo (`z-10`, `z-index: 2`)                    | **Prohibido** — usar la escala del `@theme` (`z-content`/`z-raised`/`z-nav`/`z-floating`) → [`angular-components.md`](.claude/references/angular-components.md#escala-de-apilamiento-z-index). Enforced por lint: `z-index-scale` (ESLint, `.ts`/`.html`) y `cuentoneta/z-index-scale` (Stylelint, `.css`)                                                     |
| Propiedades estáticas                                     | **Prohibidas** — usar un servicio singleton (`@Service()`)                                                                                                                                                                                                                                                                                                     |
| Config de componentes/directivas/servicios                | La configuración propia de la clase va como `private readonly` de **instancia**, no como `const` de módulo (ESLint `component-config-in-class`). Exentos: el `const` exportado y el sustituto de `enum` con su `type` derivado → [`angular-components.md`](.claude/references/angular-components.md#configuración-de-la-clase)                                 |
...
| zod en el frontend                                        | En `src/models/**` y `src/app/**` va como `import * as z from 'zod/mini'`: el paquete completo y sus subpaths versionados viajan enteros al navegador, y traer `z` por nombre anula el tree-shaking igual (ESLint `cuentoneta/no-full-zod-in-browser`). El backend (`src/api/**`) usa zod clásico → [`typescript.md`](.claude/references/typescript.md)        |
...
> Las **cuatro restricciones numéricas** las verifica ESLint sobre todo el árbol (`**/*.{ts,tsx,js,mjs}`), no solo la app: valen igual en el Studio, en `scripts/`, en `e2e/` y en `resources/`. La asimetría de las exenciones es deliberada — el callback de un `describe` y el `meta` de una story son funciones por sintaxis y no por lógica, así que medir su **largo** no dice nada; su complejidad y su anidamiento sí, y ahí no hay exención. Fuera del alcance queda `tools/**`, por el ignore global de `eslint.config.mjs`.
>
...
- **API providers del frontend:** dos archivos por dominio. `<dominio>.provider.ts` declara la interfaz `<X>Api`, la impl `Http<X>Api` y el `InjectionToken` homónimo, **tree-shakable** (`providedIn: 'root'` + `factory: () => inject(Http<X>Api)`): la implementación viaja así al chunk de quien la inyecta, y **no existe** `provide<X>Api()`. `<dominio>.mock.ts` tiene el doble (`Stub<X>Api` — devuelve canned — + `provide<X>ApiMock()`), que sí se cablea explícito y gana sobre la factory. Ver [`clean-architecture.md`](.claude/references/clean-architecture.md).
...
- **`.claude/agents/*.md`** y **`.claude/skills/<skill>/SKILL.md`** (versionados): un `description:` **no puede contener `: `** (dos puntos + espacio) sin comillas — YAML lo lee como un mapping anidado, el frontmatter queda inválido y el agente o la skill **no carga, sin emitir ningún error**. Usar un guion (`—`) en su lugar. Lo verifica el gate **`check-agents`** (`pnpm check:agents` → `scripts/check-claude-docs.ts`, que además valida las anclas a `CLAUDE.md`, las rutas citadas en las referencias y que ninguna nombre un issue —por numeral, URL o `GH-`— fuera de la allowlist de gobernanza); el fallo silencioso costó dos agentes caídos (#1874).
- **Hook `PreToolUse` de `Edit`/`Write`** (versionado en `.claude/settings.json`): [`scripts/block-issue-refs-in-comments.ts`](scripts/block-issue-refs-in-comments.ts) rechaza escribir un comentario de código bajo `src/`, `cms/` o `scripts/` que cite un issue, salvo las dos excepciones de [Comentarios](#comentarios), o que cite un identificador de hallazgo de review (`R1`, `S3`), que no tiene excepción. Los archivos que **definen** esa convención llevan una allowlist por ruta en el propio predicado: sin ella, el checker se bloquearía a sí mismo, porque sus fixtures son deliberadamente ofensivos. Es una **ayuda local, no un gate**: solo ve el texto que la herramienta agrega, no lo ya commiteado ni lo escrito por fuera.
- **Hook `commit-msg` de husky** (versionado, [`.husky/commit-msg`](.husky/commit-msg)): corre `pnpm check:findings --surface=commit` sobre el mensaje antes de crear el commit y lo rechaza si cita un identificador de hallazgo. A diferencia del hook `PreToolUse` de arriba, **éste sí tiene un gate de CI detrás** — `check-findings` — que cierra los huecos que un hook local no puede cubrir: un commit hecho con `--no-verify`, desde la web o desde otra herramienta, y el cuerpo del PR, que ningún hook local ve.
