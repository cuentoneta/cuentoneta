...
**Aliases de paths** (ver `tsconfig.json`): `@components/*`, `@mocks/*`, `@models/*`, `@utils/*`, `@test-utils`, `@sanity-types` (tipos generados de Sanity, promovidos al kernel `src/app/sanity/types.ts` — importables por frontend y backend).
