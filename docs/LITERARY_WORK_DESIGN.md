...
interface LiteraryWorkRepository {
	...
	// El listado de teasers por autor, para las sugerencias de lectura. Devuelve un registro con las
...
| Archivo                              | Rol                                                                                                                                                                                                   |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `literary-work.repository.ts`        | **Puerto**: la interfaz `LiteraryWorkRepository` (`fetchBySlug` + `fetchByAuthorSlug`) y el registro `LiteraryWorkTeaserListing`                                                                      |
...
| Aspecto         | Contrato                                                                                                                                                                                                                                                    |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Params          | `{ slug: string }` — slug del **autor**, mismo `slugSchema`. La ruta se declara **antes** de `/:slug` para que `author` no matchee como slug de obra                                                                                                        |
| Respuesta 200   | `LiteraryWorkTeaser[]` completo y ordenado por título — quién elige las que se muestran es del consumidor. Un autor sin obras (o inexistente) es `[]`, no 404: la ausencia de obras es legítima                                                             |
| Obra mal curada | Se **descarta del listado y se registra en el servidor** (slug + causa), sin afectar a las demás: el repository la reporta en `LiteraryWorkTeaserListing.malformed` y el service aplica la política de bloque accesorio. Todas mal curadas ⇒ `[]`, no error |
...
| Colección Bruno | `docs/api/bruno/literary-work/get-literary-works-by-author.bru`                                                                                                                                                                                             |

Vive en el módulo `literary-work` y no en `author` —la bibliografía es una **proyección derivada del catálogo**, una query y no una pertenencia—, siguiendo el precedente de `/story/author/:slug`. `Author` no transporta obras: la asimetría con `Collection` (que sí las transporta) es deliberada, porque una colección es una raíz de **curación** con invariante propia y la bibliografía no tiene invariante que custodiar.
