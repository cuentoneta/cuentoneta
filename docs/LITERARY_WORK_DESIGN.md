...
| Decisión              | Detalle                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Contenido             | Array de **secciones**; el teaser expone un **extracto** del arranque de la primera, no la sección entera. La navegación multi-sección en UI se difiere (Slice 2). El endpoint de lectura ([§7](#7-contrato-del-endpoint)) expone la **obra completa por slug** (`fetchBySlug`) y el **catálogo de teasers filtrable** (`fetchTeasers`, hoy con el filtro por autor para las sugerencias de lectura); la lectura de una sección puntual se difiere hasta que exista un caso de uso concreto: hoy no hay consumidor que necesite traer una sección suelta, porque todo el corpus es mono-sección.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Ruta                  | `/literary-work/:slug`, self-canonical 200 (sin redirect), un documento SSR indexable (ver [§7](#7-contrato-del-endpoint), "Estado de indexación").                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Repository            | Puerto + adaptador + doble (ver [§6](#6-repository-puerto-adaptador-y-doble)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Reading time          | **Persistido write-once**: `readingTime` por sección (`section.readingTime`) y `totalReadingTime` en el documento `literaryWork` — el texto de la obra es inmutable una vez creada, no hace falta recalcular en lectura. Poblado por un **script de backfill** ([#1959](https://github.com/cuentoneta/cuentoneta/issues/1959)), única vía de persistencia, que corre a demanda y agendado una vez por día (workflow `backfill-reading-time`); la lectura (`fetchBySlug`) deriva un **fallback puro, sin persistir**, cuando el campo todavía viene vacío (ver [§5](#5-helper-de-reading-time)). **Decisión actualizada:** la Opción B de [#1953](https://github.com/cuentoneta/cuentoneta/issues/1953) (materialización self-healing en la primera lectura del backend) queda **superseded** por el script — se prefirió simplicidad: sin patch en un endpoint público, sin estado de coalescing en el adaptador. La caché de borde de [§8](#8-estrategia-de-caché-de-borde) es una capa aparte, sobre la respuesta ya renderizada: no depende de este campo ni lo materializa. |
...
`LiteraryWork` es una raíz de agregado del contexto **Catálogo de Contenido**, junto a `Author`. Su contrato es autónomo: no extiende ningún supertipo.
...
**Decisiones de las vistas:**

- `LiteraryWorkTeaser` expone un **extracto** del arranque de la obra (`excerpt`), recortado al primer párrafo en la query; por eso el tipo no declara `readingTime` ni `position` — declararlos obligaría a inventarlos.
- `authors` es 1..N; el anonimato se expresa con el author "Anónimo" (ver [§10](#10-autoría-y-obra-anónima)).
- `totalReadingTime` es **derivado del texto** (suma de los `readingTime` de sus secciones, ensamblado en la factory) y **materializado write-once** en Sanity — ver [§5](#5-helper-de-reading-time).
- `editorialNote` es paratexto editorial de la redacción, en Markdown saneado (`SanitizedHtml`), con nombre propio.
...
Hoy **todo el corpus es mono-sección** y ninguna sección lleva `title`: el campo es opcional a propósito, y la vista de lectura lo consume con optional chaining.

### Mapeo campo a campo (CMS → dominio, responsabilidad de la ACL del repository, ver [§6](#6-repository-puerto-adaptador-y-doble))

| CMS                                        | Dominio                                 | Transformación                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------ | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `title?: string`                           | `title?: SectionTitle`                  | `createSectionTitle` (si presente); habilita `toAnchor(): Slug` para anclas de navegación                                                                                                                                                                                                                                                                                                                                                       |
...
| `epigraphs[].reference: string (markdown)` | `epigraphs[].reference?: SanitizedHtml` | **Mismo pipeline MD→HTML**; ausente o vacío en CMS → `undefined` en dominio                                                                                                                                                                                                                                                                                                                                                                     |
...
Implementados en `src/models/*.model.ts` (#1852) — primera implementación real del patrón branded-type + factory del roadmap de [`domain-model.md`](../.claude/references/domain-model.md). `Author` sigue sin brandear (roadmap #1503, sin cambios).
...
| `SectionTitle`  | `{ value; toAnchor(): Slug }` | No vacío; `toAnchor()` deriva un `Slug` válido vía `slugify`                                                           | `section-title.model.ts`  |
...
`toAnchor()` normaliza el título de la sección vía `slugify` (`lower: true, strict: true` — quita tildes y signos, kebab-case) y valida el resultado con `createSlug`: `createSectionTitle('¡Capítulo Uno!').toAnchor()` → `'capitulo-uno'` (tipado `Slug`). Su propósito es la **navegación intra-documento** en `/literary-work/:slug`: anclas estables para la tabla de contenidos y deep-links (`/literary-work/la-obra#capitulo-uno`), a consumir por la navegación multi-sección (Slice 2). Es complementaria a una eventual obtención parcial futura por sección ([§7](#7-contrato-del-endpoint), hoy diferida): el ancla desplaza dentro del documento **ya cargado**, mientras que traer una porción por request es un caso de uso todavía sin implementar.
...
- **Secciones sin `title`** no tienen ancla derivable del título — el fallback natural es el ancla posicional construida desde `position` (p. ej. `#seccion-3`); su formato exacto se decide al construir la navegación.
...
Ambos campos son **opcionales** en el schema y arrancan vacíos. El poblado lo hace un **script** ([#1959](https://github.com/cuentoneta/cuentoneta/issues/1959)) — la **única vía de persistencia** de estos campos: no ocurre en el Studio, ni por migración, ni en el read path del backend.

El script corre por dos vías: **a demanda** (`pnpm backfill:reading-time`, en seco por defecto) y **una vez por día**, agendado en el workflow `backfill-reading-time` de GitHub Actions, que lo invoca con `--no-dry-run` contra `production`. La corrida diaria es la que hace que una obra recién publicada se materialice sin intervención manual; el fallback puro de lectura cubre la ventana intermedia. Que el schedule pueda aplicar sin supervisión depende de que el patch use `setIfMissing`: la duración editorial cargada a mano no se pisa.

> **Decisión actualizada.** La planificación original ([#1953](https://github.com/cuentoneta/cuentoneta/issues/1953), Opción B) proponía poblar estos campos por **materialización self-healing en la primera lectura** del backend: el repository computaba y, si algo faltaba, lo **persistía de vuelta** (patch) antes de responder, con coalescing de escrituras concurrentes por documento para acotar el consumo de cuota en un endpoint público. Esa vía queda **superseded** por el script — patchear Sanity dentro del read path se consideró un detalle de implementación excesivo frente al beneficio: se prefirió la opción más simple (adaptador sin estado de coalescing ni dependencia de un cliente con token de escritura en el read path). El costo de esa elección —que el backfill fuera un paso operativo manual— lo salda el schedule diario descrito arriba, que lo automatiza fuera del read path en vez de dentro.
...
1. Una **sección editorial mínima** (presentación/contexto curatorial del recitado) — le da a `/literary-work/:slug` el cuerpo SSR indexable que la estrategia SEO del epic exige, y mantiene válidos `excerpt` y `sectionCount`.
...
interface LiteraryWorkRepository {
	...
	// La lectura de una sección puntual se difiere hasta que exista un caso de uso.
	fetchBySlug(slug: string): Promise<LiteraryWork | null>;
	// El catálogo de teasers, filtrable por criterios (hoy solo `author`; limit/offset y tags se
	// sumarán al mismo registro de filtro, no como firmas nuevas). Devuelve un registro con las
	// obras sanas junto a los errores de las que no se pudieron traducir: el repository reporta y no
	// decide, porque la misma obra intraducible debe tumbar el agregado que la cura (colección) y no
	// puede tumbar un bloque accesorio — la política vive en el service, que conoce el caso de uso.
...
**La ACL vive dentro del repository, no en `_utils/`.** A diferencia del resto de los módulos backend (ver [`sanity-acl.md`](../.claude/references/sanity-acl.md)), `SanityLiteraryWorkRepository` no delega la traducción raw → dominio a funciones separadas en `src/api/_utils/*.functions.ts`: la implementa como **métodos privados propios** (`mapLiteraryWork`, `mapLiteraryWorkTeaser`, `mapMetadata`, `mapSection`, `mapEpigraph`, `mapExcerpt`, `resolveTotalReadingTime`), reutilizando como building blocks los sub-mappers transversales ya existentes (`mapAuthor`, `mapResources`, `mapTags`, `mapMediaSources`). Es una **divergencia intencional**: el repository es la única frontera que conoce el shape de Sanity y entrega `LiteraryWork` de dominio listo — es la dirección arquitectónica objetivo del backend; el resto de los módulos todavía delega su traducción a `_utils/`. `InMemoryLiteraryWorkRepository` almacena y devuelve dominio (`LiteraryWork[]`), no crudo. Implementación revisada en **PR #2002** (Slice 1a).
...
| Archivo                              | Rol                                                                                                                                                                                                   |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `literary-work.repository.ts`        | **Puerto**: la interfaz `LiteraryWorkRepository` (`fetchBySlug` + `fetchTeasers`), el filtro `LiteraryWorkTeaserFilter` y el registro `LiteraryWorkTeaserListing`                                     |
...
### `GET /literary-work` — el catálogo, filtrable por query params

| Aspecto         | Contrato                                                                                                                                                                                                                                              |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Query params    | `{ author?: string }` — slug del **autor**, mismo alfabeto que el `slugSchema` de path (400 si lo viola). El filtrado va por **query params, no por sub-rutas**: cada criterio futuro (limit/offset para paginación, tags) es un campo más del schema |
| Respuesta 200   | `LiteraryWorkTeaser[]` ordenado por título — sin filtros, el catálogo entero; con `author`, sus obras. Quién elige las que se muestran es del consumidor. Un filtro sin resultados (o un autor inexistente) es `[]`, no 404: la ausencia es legítima  |
| Obra mal curada | Se **descarta del listado y se registra en el servidor** (slug + causa), sin afectar a las demás: el repository la reporta en `LiteraryWorkTeaserListing.malformed` y el service aplica la política de listado. Todas mal curadas ⇒ `[]`, no error    |
| Registro        | El mismo mount de `/literary-work`; la caché de borde lo cubre por comodín, con la misma cadencia editorial que la obra                                                                                                                               |
| Colección Bruno | `docs/api/bruno/literary-work/get-literary-works.bru`                                                                                                                                                                                                 |

El listado por autor vive acá y no en el módulo `author` —la bibliografía es una **proyección derivada del catálogo**, una query y no una pertenencia—. `Author` no transporta obras: la asimetría con `Collection` (que sí las transporta) es deliberada, porque una colección es una raíz de **curación** con invariante propia y la bibliografía no tiene invariante que custodiar.

`publishedAt` se resuelve en la query con `coalesce(publishedAt, _createdAt)` (`src/api/_queries/literary-work.query.ts`): la fecha de publicación cae al `_createdAt` del documento cuando el campo editorial no está cargado, en vez de quedar `undefined`.

### Lectura de sección puntual — trabajo futuro diferido

La obra se sirve **completa** por slug: no existe hoy ninguna variante de obtención parcial. Se difiere hasta que exista un caso de uso concreto: hoy no hay consumidor que necesite traer una sección suelta, porque todo el corpus es mono-sección. Cada sección ya transporta su **`position`** (0-based, [§3](#3-shape-de-sección)) — identidad numérica estable, independiente del índice del array de la respuesta —, condición que cualquier obtención parcial futura podría reutilizar sin cambiar el shape de sección. La query GROQ de obtención parcial (`literaryWorkSectionBySlugQuery`) ya proyecta `editorialNote` junto al resto de la metadata, aunque el endpoint que la consuma siga sin implementarse.

El SSR de `/literary-work/:slug` (Slice 1) consume la forma completa; la navegación multi-capítulo (Slice 2) también parte de la obra completa ya cargada (ver anclas intra-documento, [§4](#4-value-objects)).

> **Decisión (Slice 1c) — slug inexistente → 404 propio.** Un slug inexistente devuelve **404 con envelope `{ error }`**: el service lanza `LiteraryWorkNotFoundError` (error tipado por operación) y el controller lo traduce a esa respuesta, en vez de dejar que degrade al 500 sin body estructurado del `onError` global. Es el contrato correcto para un recurso ausente.
...
- **Métodos**: `SectionTitle.toAnchor()` no viaja por JSON — tipear la respuesta como `LiteraryWork` produciría un error en runtime al invocarlo.
...
**Alternativa evaluada y descartada:** volver el modelo _serialization-transparent_ demoviendo `toAnchor()` a función pura standalone y compartiendo el tipo de dominio como contrato de wire. Se descarta porque renuncia al modelo rico recién construido y porque las factories ya existen y están testeadas — el costo de la rehidratación es marginal frente a recuperar métodos, brands re-validados e inmutabilidad en el cliente.

> **Estado de indexación.** `/literary-work/:slug` es superficie SSR **indexable**: `literary-work.page.ts` declara el combo `LiteraryWorkMetaTagsDirective` + `LiteraryWorkStructuredDataDirective` —la forma indexable que valida `seo-host-directives.spec.ts`— y emite canonical self-referencial, meta/OG por página y JSON-LD `Article` + `BreadcrumbList`. Las invariantes de la ruta viven en `e2e/seo/literary-work.spec.ts`, y el smoke de indexado (`scripts/seo-smoke.ts`) la ejercita en su baseline.
...
| Elemento          | Diseño                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Qué se cachea     | La **respuesta entera** de las dos superficies de lectura de una obra — la página SSR de `/literary-work/:slug` y el JSON de `GET /api/literary-work/:slug` —, no un derivado intermedio.                                                                                                                                                                                                                                                                                             |
| Página SSR        | Middleware `ssrCacheControl` (`src/api/_middleware/ssr-cache-control.middleware.ts`), montado solo en `/literary-work/*`. **Guarda anti-CSR:** bufferiza e inspecciona el body (`c.res.clone().text()`) buscando el marcador `ng-server-context="ssr"` antes de cachear — el fallback CSR degradado responde con el mismo `200 text/html` sin ningún header distintivo, así que cachearlo serviría una página vacía durante todo el TTL. No cachea ante respuestas no-200.            |
...
La ACL **no filtra ni normaliza** la referencia: el dominio recibe `authors: [Author('Anónimo')]` como cualquier otra autoría. La invariante del agregado pasa a ser `authors.length >= 1`.
...
| VOs (`Slug`, `WordCount`, `ReadingTime`, `Markdown`, `SanitizedHtml`, `SectionTitle`) + `createIsoDateTime`              | ✅ Implementados con specs                                   | Consume                                                            |
...
| Reading time: fallback puro en lectura + script de backfill                                                              | Contrato ([§5](#5-helper-de-reading-time))                   | ⚙️ Repository implementa el fallback; script agendado a diario     |
| Endpoint `GET /literary-work/:slug` + `.bru`                                                                             | Contrato ([§7](#7-contrato-del-endpoint))                    | ⚙️ Implementa                                                      |
| Frontend (`LiteraryWorkApi` + `LiteraryWorkDto` + rehidratación en el provider, ruta `/literary-work/:slug`, página SSR) | Contrato ([§7](#7-contrato-del-endpoint))                    | ⚙️ Implementa                                                      |
| Caché de borde `/literary-work` (middleware + endpoint), con SWR y sin purga (#1856)                                     | Contrato ([§8](#8-estrategia-de-caché-de-borde))             | ✅ Implementado                                                    |
