...
| Purge-on-publish                  | Módulo `src/api/modules/cache-purge/` (controller → service → repository, patrón estándar de [`sanity-acl.md`](../.claude/references/sanity-acl.md)). Webhook de Sanity → `POST /api/cache-purge` → valida la firma HMAC del **cuerpo crudo** (`@sanity/webhook`, `isValidSignature`, antes de parsear el shape) → `purgeLiteraryWorkCache` → `invalidateByTag('literary-work:<slug>')` de `@vercel/functions` (sin token explícito: corre dentro de la función desplegada, usa el contexto de Vercel). Firma inválida/ausente → 401; shape inválido → 400. El secreto compartido es `SANITY_WEBHOOK_SECRET` (`environment.sanityWebhookSecret`); vacío por defecto ⇒ fail-closed (toda firma falla sin secreto configurado). |
