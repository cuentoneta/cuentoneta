...
> **Estado de implementación:** el **mapper puro** que lee y computa (`mapLiteraryWork` en `src/api/_utils/literary-work.functions.ts`, más los helpers de materialización — `planReadingTimeMaterialization` y afines — en `reading-time-materialization.functions.ts`) ya aterrizó en `develop`. Lo que sigue pendiente es el **repository** que ejecuta el patch de escritura contra Sanity — y el `?section=N` eficiente que se apoya en él para traer solo el body de la sección pedida más el total ya persistido ([§7](#7-contrato-del-endpoint)) —, que se implementan en el increment de backend rebaseado sobre el PR [#1929](https://github.com/cuentoneta/cuentoneta/pull/1929) (read-side diferido).
