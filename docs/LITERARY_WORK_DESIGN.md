...
- **El bypass del cuerpo lo hace `LiteraryWorkProseComponent`**, no la página: ese componente es el dueño del HTML de la prosa —de él cuelgan el binding y las reglas tipográficas de los nodos que el pipeline emite, ancladas a su selector de elemento desde `src/assets/css/prose.css`—. La página conserva el bypass de los epígrafes, que pinta su propia plantilla.
