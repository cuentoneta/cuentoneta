...
— y el total del agregado sigue siendo `sumReadingTimes(sections.map(s => s.readingTime))`. Lo que cambia frente al transform-on-read puro: cuando el repository backend lee una obra a la que le falta `readingTime`/`totalReadingTime`, además de computarlos (con el `countWords` real del dominio, "fuera del CMS") los **persiste de vuelta** en Sanity vía un patch (`@sanity/client` con token de escritura) antes de responder. Las lecturas siguientes de esa obra encuentran el valor ya persistido y lo sirven directo, sin recomputar. Como el texto es inmutable, la regla "falta → computar y persistir" alcanza — no hace falta invalidar por hash ni por `_rev`, y cubre obras nuevas automáticamente sin paso manual en el Studio ni webhook. Antes de armar el patch, el adaptador cortocircuita las obras ya materializadas (`totalReadingTime` y todos los `readingTime` de sección presentes) con un chequeo barato —sin re-parsear Markdown—, así el caso común (post primera lectura / post backfill) no paga cómputo en el hot path. Y como el endpoint de lectura es **público**, el adaptador Sanity ([§6](#6-repository-puerto-adaptador-y-doble)) **coalesce las materializaciones concurrentes por documento** (estado de instancia del repository): una ráfaga sobre la misma obra sin materializar comparte una sola escritura en lugar de disparar una por request. El coalescing entre requests es efectivo solo si el repository se compone como **singleton** (decisión del edge — service/controller), ya que el registro de en-vuelo es de instancia, no global. La escritura ya es idempotente (`setIfMissing`); el coalescing solo acota el consumo de cuota. El backfill batch ([#1959](https://github.com/cuentoneta/cuentoneta/issues/1959)) pre-materializa para que el caso común no escriba en lectura.
...
| Archivo                              | Rol                                                                                                                                                                                                                                                                      |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `literary-work.repository.ts`        | **Puerto**: la interfaz `LiteraryWorkRepository` + `projectSingleSection` (proyección dominio→dominio; es lógica del _contrato_ —parte de lo que significa implementar `fetchSectionBySlug`—, no un detalle de Sanity, así que la comparten ambos adaptadores desde acá) |
| `literary-work.repository.sanity.ts` | **Adaptador Sanity**: `SanityLiteraryWorkRepository` + su ACL privada (traducción raw→dominio) + materialización write-on-read                                                                                                                                           |
