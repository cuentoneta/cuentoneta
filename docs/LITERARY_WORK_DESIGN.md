...
| Ruta                  | `/read/:slug`, self-canonical 200 (sin redirect), un documento SSR indexable (ver [§7](#7-contrato-del-endpoint), "Estado de indexación").                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
> **Estado de indexación.** `/read/:slug` es superficie SSR **indexable**: `read.page.ts` declara el combo `ReadMetaTagsDirective` + `ReadStructuredDataDirective` —la forma indexable que valida `seo-host-directives.spec.ts`— y emite canonical self-referencial, meta/OG por página y JSON-LD `Article` + `BreadcrumbList`. Las invariantes de la ruta viven en `e2e/seo/read.spec.ts`, y el smoke de indexado (`scripts/seo-smoke.ts`) la ejercita en su baseline.
