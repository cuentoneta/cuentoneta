...
- **El bypass del cuerpo lo hace `LiteraryWorkSectionBodyComponent`**, no la página: ese componente es el dueño de ese HTML —de él cuelgan el binding y las reglas tipográficas de los nodos que el pipeline emite, ancladas a su selector de elemento desde `src/assets/css/literary-work-section-body.css`—. La página conserva el bypass de los epígrafes, que pinta su propia plantilla.
