...
| Página SSR                        | Middleware `ssrCacheControl` (`src/api/_middleware/ssr-cache-control.middleware.ts`), montado solo en `/read/*`. **Guarda anti-CSR:** bufferiza e inspecciona el body (`c.res.clone().text()`) buscando el marcador `ng-server-context="ssr"` antes de cachear — el fallback CSR degradado responde con el mismo `200 text/html` sin ningún header distintivo, así que cachearlo serviría una página vacía durante todo el TTL. No cachea en no-producción ni ante respuestas no-200.                                                                                                                                                                                                                                                                                                                                                                                                           |
| Endpoint JSON                     | `GET /api/literary-work/:slug` emite los mismos headers **inline en el controller** (`literary-work.controller.ts`) tras un 200 en producción: el JSON es determinístico y no tiene el riesgo de fallback CSR de la página, así que no necesita el guard de body.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Headers                           | `Vercel-CDN-Cache-Control` lleva el TTL largo (`s-maxage` + `stale-while-revalidate=604800`, 7 días) — lo honra únicamente el CDN de Vercel, no se reenvía al browser. `Cache-Control` del browser es `public, max-age=0, must-revalidate` (siempre fresco): el purge-on-publish solo alcanza el CDN, así que el browser no puede quedarse con un TTL largo. Cada respuesta cacheable lleva `Vercel-Cache-Tag: literary-work:<slug>` — una única key agrupa **todas** las variantes `?section=N` de una obra, para que una purga cubra la obra completa de una vez. Fuente única de estos headers: `src/api/_helpers/cache-control.ts` (`applyReadCacheHeaders` + `literaryWorkCacheTag`), consumida por ambas superficies.                                                                                                                                                                     |
| `s-maxage`                        | Interruptor de entorno `READ_CACHE_S_MAXAGE` (`EnvironmentConfig.readCacheSMaxAge`), con default conservador de **5 minutos** si la variable no está seteada o es inválida. El valor decidido para producción es **un año** (`31536000`) — ver "paso operativo pendiente" abajo.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Purge-on-publish                  | Módulo `src/api/modules/cache-purge/` (controller → service → repository). Ojo: el "repository" acá es un **adaptador a un efecto externo de Vercel** (`invalidateByTag`), no un repository de dominio con ACL GROQ→mapper como el de `sanity-acl.md`; comparte la forma de capas, no el patrón de traducción de datos. Webhook de Sanity → `POST /api/cache-purge` → valida la firma HMAC del **cuerpo crudo** (`@sanity/webhook`, `isValidSignature`, antes de parsear el shape) → `purgeLiteraryWorkCache` → `invalidateByTag('literary-work:<slug>')` de `@vercel/functions` (sin token explícito: corre dentro de la función desplegada, usa el contexto de Vercel). Firma inválida/ausente → 401; shape inválido → 400. El secreto compartido es `SANITY_WEBHOOK_SECRET` (`environment.sanityWebhookSecret`); vacío por defecto ⇒ fail-closed (toda firma falla sin secreto configurado). |
| Load-bearing con `s-maxage` largo | El purge-on-publish es lo que hace segura la decisión de `s-maxage=1 año`: sin él, una edición o corrección editorial de una obra ya publicada quedaría stale en el CDN hasta por un año. Por eso el TTL largo se activa **recién cuando el purge está verificado operando end-to-end**, no antes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |

### Paso operativo pendiente

1. Dar de alta el webhook en Sanity: una proyección GROQ que, ante la publicación de una obra, emita `{ "slug": slug.current }`, apuntando a `POST /api/cache-purge` con el secreto compartido para la firma (`SANITY_WEBHOOK_SECRET`).
2. Verificar el purge end-to-end en producción (editar una obra publicada, confirmar que la respuesta cacheada se invalida).
3. Recién entonces, subir `READ_CACHE_S_MAXAGE` a `31536000` (un año) en el entorno de producción de Vercel.

Hasta que se complete el paso 1, producción opera con el `s-maxage` conservador (5 minutos): el riesgo de servir contenido stale es bajo, a costa de menor hit rate.
...
| Caché de borde `/read` (middleware + endpoint) + purge-on-publish (#1856)                                       | Contrato ([§8](#8-estrategia-de-caché-de-borde))             | ✅ Implementado (webhook en Sanity y bump a `s-maxage=1 año` pendientes, ver §8) |
